...
const maintFromDb = (r) => ({ id: r.id, kundenId: r.kunden_id, kunde: r.kunde, modell: r.modell, seriennummer: r.seriennummer, installation: r.installation, letzteWartung: r.letzte_wartung, naechsteWartung: r.naechste_wartung, notiz: r.notiz, aktiv: r.aktiv !== false, geplantDatum: r.geplant_datum });
...
export default function Dashboard() {
  ...
  async function updateMaintenanceItem(id, patch) {
    const { error } = await supabase.from('maintenance_items').update(patch).eq('id', id);
    if (error) setErrorMsg(error.message); else loadAll();
  }
